import { renderBuffer, openAnim } from "./codec.ts";
...
const staticFiles = [
    { name: "PNG",  file: "fox.png" },
    { name: "JPEG", file: "fox.jpg" },
    { name: "BMP",  file: "fox.bmp" },
    { name: "WebP", file: "fox.webp" },
];
const bufs = new Map<string, Uint8Array>();
for (const f of staticFiles) bufs.set(f.file, new Uint8Array(fs.readFileSync(f.file)));
const gifBuf = new Uint8Array(fs.readFileSync("teseractor.gif"));
const webpBuf = new Uint8Array(fs.readFileSync("teseractor.webp"));

interface AnimResult { name: string; firstFrameMs: number; totalFrames: number; avgRenderMs: number; avgFps: number; minRenderMs: number; maxRenderMs: number; totalTimeMs: number; }
interface MaxResult { name: string; srcW: number; srcH: number; termW: number; termH: number; frames: number; totalMs: number; fps: number; avgRender: number; minRender: number; maxRender: number; }
...
const staticStats: { name: string; parseMs: number; renderMs: number; srcW: number; srcH: number; termW: number; termH: number }[] = [];
renderBuffer(bufs.get("fox.png")!, { termW: QW, termH: QH }); // warm canvas

for (let i = 0; i < staticFiles.length; i++) {
    const f = staticFiles[i]!;
    const buf = bufs.get(f.file)!;
    let pSum = 0, rSum = 0, sw = 0, sh = 0;
    for (let r = 0; r < 10; r++) { const { metrics: m } = renderBuffer(buf, { termW: QW, termH: QH }); pSum += m.parseMs; rSum += m.renderMs; sw = m.imgW; sh = m.imgH; }
    const pAvg = pSum / 10, rAvg = rSum / 10, total = pAvg + rAvg;
    staticStats[i] = { name: f.name, parseMs: pAvg, renderMs: rAvg, srcW: sw, srcH: sh, termW: QW, termH: QH };

    const { ansi } = renderBuffer(buf, { termW: QW, termH: QH });
    const col = (i % 2) * (QW + 1);
    ...
    const statsLine = `${f.name.padEnd(5)} parse ${pAvg.toFixed(1)}ms render ${rAvg.toFixed(1)}ms → ${(1000/total).toFixed(0)}fps`;
...
async function runAnim(name: string, buf: Uint8Array, speed: number): Promise<AnimResult> {
    ...
    if (!player) return { name, firstFrameMs: 0, totalFrames: 0, avgRenderMs: 0, avgFps: 0, minRenderMs: 0, maxRenderMs: 0, totalTimeMs: 0 };
    ...
    return { name, firstFrameMs: ffMs, totalFrames: frameCount,
...
async function runMax(name: string, buf: Uint8Array): Promise<MaxResult> {
    ...
    if (!player) return { name, srcW: 0, srcH: 0, termW: AW, termH: AH, frames: 0, totalMs: 0, fps: 0, avgRender: 0, minRender: 0, maxRender: 0 };

    // get source dimensions from first frame
    const first = player.next();
    if (!first) { player.close(); return { name, srcW: 0, srcH: 0, termW: AW, termH: AH, frames: 0, totalMs: 0, fps: 0, avgRender: 0, minRender: 0, maxRender: 0 }; }
    ...
    while ((performance.now() - tStart) < 10000) {
        const n = player.next();
        if (!n) { player.rewind(); continue; }
        const tr = performance.now();
        const r = player.renderFrame(n.frameIndex);
        ...
        // yield every 20 frames to keep display breathing
        if (frameCount % 20 === 0) await new Promise(r => setTimeout(r, 0));
    }

    player.close();
    const totalMs = performance.now() - tStart;
    return { name, srcW: sw, srcH: sh, termW: AW, termH: AH, frames: frameCount, totalMs,
...
const FMT = ["PNG", "JPEG", "BMP", "WebP", "GIF", "HDR", "PNM"];
console.log(`=== Summary (${TW}x${TH} terminal) ===\n`);

console.log("Static (10x avg):");
for (const s of staticStats) {
    const t = s.parseMs + s.renderMs;
    console.log(`  ${s.name.padEnd(5)} src ${s.srcW}x${s.srcH} → term ${s.termW}x${s.termH}`);
    console.log(`  ${" ".repeat(5)} parse ${s.parseMs.toFixed(1)}ms render ${s.renderMs.toFixed(1)}ms total ${t.toFixed(1)}ms → ${(1000/t).toFixed(0)}fps`);
}

console.log("\nAnimated (10s, native speed):");
for (const r of [gifAnim, webpAnim]) {
    console.log(`  ${r.name.padEnd(5)} term ${TW}x${Math.min(TH - 4, TH - 2)}`);
    console.log(`  ${" ".repeat(5)} first-frame ${r.firstFrameMs.toFixed(0)}ms | ${r.totalFrames}frames @ ${r.avgFps.toFixed(0)}fps`);
...
for (const r of [gifMax, webpMax]) {
    ...
    console.log(`  ${" ".repeat(5)} ${r.frames}frames → ${r.fps.toFixed(0)}fps (${r.totalMs.toFixed(0)}ms total)`);
