...
#define NAPI_CALL(env, call) do { napi_status s = (call); if (s != napi_ok) { napi_throw_error(env, NULL, #call " failed"); return NULL; } } while(0)
...
static int read_int32(napi_env env, napi_value obj, const char* name, int32_t* out) {
    napi_value val; napi_valuetype type;
...
static void read_config(napi_env env, napi_value obj, CodecConfig* cfg) {
    ...
    cfg->term_w = 80; cfg->term_h = 24; cfg->speed = 1.0f;
...
static napi_value create_metrics_object(napi_env env, CodecMetrics* m) {
    ...
    napi_create_double(env, (double)m->parse_ms, &val); napi_set_named_property(env, obj, "parseMs", val);
    napi_create_double(env, (double)m->inflate_ms, &val); napi_set_named_property(env, obj, "inflateMs", val);
    napi_create_double(env, (double)m->defilter_ms, &val); napi_set_named_property(env, obj, "defilterMs", val);
    napi_create_double(env, (double)m->render_ms, &val); napi_set_named_property(env, obj, "renderMs", val);
    napi_create_int32(env, m->img_w, &val); napi_set_named_property(env, obj, "imgW", val);
    napi_create_int32(env, m->img_h, &val); napi_set_named_property(env, obj, "imgH", val);
    napi_create_int32(env, m->frame_count, &val); napi_set_named_property(env, obj, "frameCount", val);
    napi_create_int32(env, m->frame_delay_ms, &val); napi_set_named_property(env, obj, "frameDelayMs", val);
    napi_create_int32(env, m->format, &val); napi_set_named_property(env, obj, "format", val);
    return obj;
}
...
static napi_value render_buffer(napi_env env, napi_callback_info info) {
    ...
    CodecConfig cfg;
    if (argc >= 2) read_config(env, argv[1], &cfg);
    else { memset(&cfg,0,sizeof(cfg)); cfg.term_w=80; cfg.term_h=24; cfg.speed=1.0f; }

    CodecMetrics m; int32_t err = 0;
    char* ansi = codec_render_buffer((char*)data, (int32_t)len, &cfg, &m, &err);

    if (err != 0) {
        napi_throw_error(env, NULL, ansi ? ansi : "render failed");
        if (ansi) codec_free_string(ansi);
        return NULL;
    }
...
static napi_value anim_open_buffer(napi_env env, napi_callback_info info) {
    ...
    CodecConfig cfg; memset(&cfg, 0, sizeof(cfg));
    cfg.term_w = 80; cfg.term_h = 24; cfg.speed = 1.0f;
    if (argc >= 2) read_config(env, argv[1], &cfg);
...
static napi_value anim_rewind(napi_env env, napi_callback_info info) {
    ...
    napi_get_cb_info(env, info, &argc, argv, NULL, NULL);
    int32_t handle; napi_get_value_int32(env, argv[0], &handle);
